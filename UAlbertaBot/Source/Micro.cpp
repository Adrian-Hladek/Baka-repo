...
void Micro::SmartAttackPatrol(BWAPI::Unit attacker, const BWAPI::Position& targetPosition)
{
    ...
    if (((currentCommand.getType() == BWAPI::UnitCommandTypes::Move) && (currentCommand.getTargetPosition() == targetPosition) && attacker->isMoving() ) || ((currentCommand.getType() == BWAPI::UnitCommandTypes::Attack_Move) && (currentCommand.getTargetPosition() == targetPosition) && attacker->isMoving()))
    {
        //std::cout << "ak už dostal rovnaký command ignorujem\n"; // chost
        return;
    }

    if (attacker->getDistance(targetPosition) < 100)
    {
        //std::cout << "range ok\n";
        return;
    }
