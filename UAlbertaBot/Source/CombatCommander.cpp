...
void CombatCommander::updateIdleSquad()
{
    ...
    for (auto & unit : m_combatUnits)
    {
        ...
        {
            //std::cout << unit->getType() << "Added to idle\n";
...
BWAPI::Position CombatCommander::getMainAttackLocation()
{
    ...
    if (BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Zerg && BWAPI::Broodwar->getFrameCount() < 3500)
    {
     ...
     // Pre choice: Attacks overlord in Corsair strategy
    
    if (Config::Strategy::StrategyName == "Protoss_Corsair")
    {
        for (auto& unit : BWAPI::Broodwar->enemy()->getUnits())
        {
    ...
    // Fourth choice: Check unchecked bases
    if (enemyBaseLocation) {
